...
import {addMainCourse,getMainCourses} from "../../actions/mainCourseActions";
...
class AddMainCourse extends Component{
    constructor(props){
        super(props);
        this.state={
            id:'',
    ...
    addNewMainCourse=(e)=>{ 
        ...
        _this=this;
        if(name===''||price===''||description===''||picture===''||subcategory===''){
            this.setState({
                error:true
            });
        }
            ...
            this.props.addMainCourse(formData);
        ...
        try {
            await api.get('/api/get/main-courses')
                .then(response => {
                    ...
                    idString=totalOfItems+1+'ADDEDMNCRS_'+customRandomString;//console.log(idString); 
                })
                .catch(error => {
                    console.log(error);
            });
            ...
            console.log('An error occurs in AddMainCourse.componentDidMount');
            console.log(error);
...
export default connect(mapStateToProps,{clearIngredientsByDish,deleteIngredientDish,setNextIdDishIngredient,setDishId,setAddIngredient,addMainCourse,getMainCourses})(AddMainCourse);